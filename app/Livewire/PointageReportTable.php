...
class PointageReportTable extends Component implements HasForms, HasTable
{
    ...
    public function getTableQuery(): Builder
    {
        $query = Pointage::query()
            ...
            ->join('pointage_user', 'pointages.id', '=', 'pointage_user.pointage_id')
            ->join('users', 'pointage_user.user_id', '=', 'users.id')
            ->select([
                'pointages.id',
                'pointages.date',
        ...
        if ($this->reportType === 'weekly') {
            $query->select([
                DB::raw('pointages.id || "-" || projects.id || "-" || users.id as id'),
            ...
            ->groupBy(DB::raw('strftime("%Y%W", pointages.date)'), 'projects.id', 'users.id');
            ...
            ->groupBy(DB::raw('strftime("%Y-%m", pointages.date)'), 'projects.id', 'users.id');
        }
            ...
            Action::make('export')
                ...
                ->action(function ($record) {
                    ...
                    return response()->streamDownload(function () use ($record) {
                        ...
                        echo "Date, Projet, Employé, Heures Travaillées, Heures Supplémentaires, Coefficient, Heures Pondérées, Statut\n";
                        
                        // Properly escape fields and add spaces after commas
                        $date = $record->date;
                        $project = str_replace(',', ' ', $record->project_name);
                        $user = str_replace(',', ' ', $record->user_name);
                        ...
                        echo "\"{$date}\", \"{$project}\", \"{$user}\", {$heures}, {$heures_supp}, {$coef}, {$heures_pond}, \"{$status}\"\n";
